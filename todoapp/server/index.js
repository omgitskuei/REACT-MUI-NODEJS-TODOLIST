...
/* Set up routes with files uploads */
const upload = multer({ storage });
app.post("/auth/register", upload.single("picture"), register); // register is a controller
// Note, the above route would normally be in the ./routes folder but this one requires var upload to work.
...
const PORT = process.env.PORT || 6001; // port 6001 is backup in case 3001 doesn't work
mongoose.connect(process.env.MONGO_URL, {
  ...
  console.log(`Failed to connect - error was thrown: ${error}`)
});
